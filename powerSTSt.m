function [RES,TIME,its,TMv, Tits,RHO,Xo,Xe] = powerSTSt (S, count,x0, tol) % Applying skew-symmetric power to S=S-[U V][Sigma 0;0 Sigma][V -U]^{T}
  Xo = []; Xe = []; RES =[]; RHO = []; its = []; Mv = []; TIME=[];
%xo = x1; Sxo = S*xo; mv = 1;  
n = size(S,1);
m =1; 
